...
class UsersModel extends Model{
	...
	public function userList(){
		...
		if(isset($param['uid']) && $param['uid']){
			...
			$where[] = array('users.uid','=',trim($param['uid']));
		...
		//查询符合条件的数据
		$userList = $this->field('users.*,user_total.balance,user_total.total_balance')->join('user_total','users.id = user_total.uid','left')->where($where)->order('users.id','desc')->select()->toArray();
		//部分元素重新赋值
	...
	public function editView(){
		...
		$data['susername'] = $this->where('id', $data['sid'])->value('username');
		//获取该用户所有上级
