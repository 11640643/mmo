...
// 		$lang	        = !empty($param['lang']) ? $param['lang'] : 'en';	// 语言类型
	...
	public function getuserinfo($lang){
		...
		$userinfo = $this->where(array('users.id'=>$uid))->findOrEmpty();
	...
	public function withdrawalrecord($lang){
		...
		->withAttr('state', function($value, $data) use ($lang) {
			if($lang == 'en') $stateTxt =config('custom.state_en')[$value];
...
// 		}
	...
	public function trade_details($lang){
		...
		$userArr  =	explode(',',auth_code($param['token'],'DECODE'));
		$uid      =	$userArr[0];
		$type 	  = empty($param['type']) ? 0 : intval($param['type']); // 1：收入； 2：支出；
		...
		$where['uid'] = $uid;
		if($type == 1){
			$where['trade_type'] = [1,3,4,5,6,7,9,13];
		} 
		if($type == 2){
			$where['trade_type'] = [2,8,14];
		}
    ...
    public function pledge($lang){
		...
		$new_user_pledge_data	= [
			...
			'amount'    		=> $amount,          //质押总数量
			'day'       		=> $day,             //质押天数
			'pledge_income'     => $pledge_income,   //质押收益
			'start_time'        => time(),
